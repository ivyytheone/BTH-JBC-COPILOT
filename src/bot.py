...
prompts.add_data_source(
    AzureAISearchDataSource(
        AzureAISearchDataSourceOptions(
            ...
            indexName='vector-jbc-testdata', #Set an alive and existing index name for AzureSearch to search for.
...
async def on_members_added(context: TurnContext, state: TurnState):
    await context.send_activity("Hej, hur kan jag hjälpa dig idag? 😊") #Tweak this to how you see fits.
...
async def on_error(context: TurnContext, error: Exception):
    ...
    traceback.print_exc() #Error handling, this can be improved and should be setup in production through Azure Application Insights.
